...
Pacf(data$LL)
ur.df(data$LL, type="drift", lags = 1, 
      selectlags = "Fixed")

d1L<-diff(data$LL, differences=1)
...
#--------------------------------------Arima&Garch------------------------------
eacf(d1L)
ARMAmodel <- Arima(data$LL, c(0,1,1), include.constant =TRUE, method = c("CSS-ML"))  
coeftest(ARMAmodel)
summary(ARMAmodel)
Box.test(residuals(ARMAmodel), lag = 7, type = c("Ljung-Box"), fitdf = 1)
#p-value =  0.09392 ~ 0.1 нулевая гипотеза не отвергается и в остатках нет автокорреляции
forecast_ARMA<-forecast(ARMAmodel, h=70, level = c(0.90,0.95))
plot(forecast_ARMA)
forecast_ARMA
write.csv(forecast_ARMA, "ARMA_Lanzhou_forecast.csv")
...
Box.test(strese$V1, lag = 7, type = c("Ljung-Box"), fitdf = 1) #p-value = 0.04678
Box.test(strese$V1^2, lag = 7, type = c("Ljung-Box"), fitdf = 1) #p-value = 0.9611

prognoz <- ugarchforecast(modele, n.ahead = 70, ci=0.95)
plot.ts(prognoz@forecast$seriesFor)
Yforecast<-c(d1L, prognoz@forecast$sigmaFor)
plot.ts(Yforecast)
write.csv(prognoz@forecast$sigmaFor, "GARCH_prognoz_Lanzhou.csv")
...
df2 = data.frame(ydL, xOG, xISF)
#---------------------------------------VAR model-------------------------------


ARMAmodel <- Arima(data$LL, c(0,1,4), include.constant =TRUE, method = c("CSS-ML"))  
...
var<-VAR(df1, p = 9, type = "const", exogen = exog)
...
Box.test(residuals(ARMAmodelFC), lag = 7, type = c("Ljung-Box"), fitdf = 1) #p-value = 0.7527
...
write.csv(forecastVARef$fcst$ydL, "forecast_VARef.csv")
...
datapr <- data.frame(df$data2.data7.Date, df$d1L)
datapr
datapr <- datapr %>% 
  rename("ds" = "df.data2.data7.Date",
         "y" = "df.d1L")
datapr$ds <- anytime(datapr$ds)

model1 <- prophet(datapr)
future1 <- make_future_dataframe(model1, periods = 70)
tail(future1)

forecast1 <- predict(model1, future1)
tail(forecast1[c('ds','yhat','yhat_lower','yhat_upper')])
write.csv(forecast1, "prophet_d1_Lanzhou.csv")
...
xy <- as.xts(datapr$y, order.by = datapr$ds, frequency=NULL, RECLASS=FALSE)
dataETS <- data.frame(xy, datapr$ds)

Lanz <- window(xy,datapr$ds)
