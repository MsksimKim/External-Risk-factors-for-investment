...
Pacf(data$LYan)
ur.df(data$LYan, type="drift", lags = 1, 
      selectlags = "Fixed")

d1L<-diff(data$LYan, differences=1)
...
d1DJIA <- diff(data$data_pr3.data_pr2.DJIA.758.2023..1.1105., differences = 1)
...
#--------------------------------------Arima&Garch------------------------------
eacf(d1L)
ARMAmodel <- Arima(data$LYan, c(0,1,1), include.constant =TRUE, method = c("CSS-ML"))  
coeftest(ARMAmodel)
summary(ARMAmodel)
Box.test(residuals(ARMAmodel), lag = 7, type = c("Ljung-Box"), fitdf = 1)
#p-value =  0.7165> 0.1 нулевая гипотеза не отвергается и в остатках нет автокорреляции
...
specse<- ugarchspec(mean.model = list(armaOrder = c(0,1), include.mean = TRUE),
                    variance.model = list(model = 'sGARCH', garchOrder = c(3,0)),
                    distribution.model = 'std')

modele <- ugarchfit(data = data$LYan, spec = specse)
modele@fit$LLH
...
write.csv(residuals(modele, standardize="TRUE"), "garch_modeleYantai.csv")
strese <-read.csv("garch_modele1.csv", sep=',', dec='.')

Box.test(strese$V1, lag = 7, type = c("Ljung-Box"), fitdf = 1) #p-value = .1935
Box.test(strese$V1^2, lag = 7, type = c("Ljung-Box"), fitdf = 1) #p-value = .4332
#-----------------------structural break test----------------------------
AdjLo <- ARMAmodel$fitted + residuals(ARMAmodel)/modele@fit$sigma*sd(residuals(modele))
d1AdjLo <- diff(AdjLo, differences = 1)


d1j<-d1AdjLo[seq(1,860)]

plot.ts(d1j)

d1_l1j <- c(0,d1j[1:length(d1j)-1])
d1_l2j <- c(0,0,d1j[2:length(d1j)-2])
d1_l3j <- c(0,0,0,d1j[3:length(d1j)-3])
d1_l4j <- c(0,0,0,0,d1j[4:length(d1j)-4])
#H0 - no structural breaks
#-------------------Sup-F test (supWald) +------------------
statj <- Fstats(d1j ~ d1_l1j + d1_l2j + d1_l3j + d1_l4j,from = 0.1, to = NULL)
plot(statj, alpha = 0.01)
lines(breakpoints(statj))
aj<-breakpoints(statj)
aj$breakpoints #87
sctest(statj, type = "supF") 

#----------------------------------graphs for comparison------------------------
data$data7.Date <- anytime(data$data7.Date)
data$LSha <- as.numeric(data$LSha)
...
#in ccf negative for x leads to y

#1
ccf(d1L, d1E, lag.max = 14, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1E, order = 8) # < 2.2e-16 d1E is caused by d1L

grangertest(d1E, d1L, order = 8) # 0.0001088 d1L is caused by d1E

#2
ccf(d1L, d1OG, lag.max = 23, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1OG, order = 8) # 3.692e-05

grangertest(d1OG, d1L, order = 22) #  0.02248 OG slightly ganger causes L

#3
ccf(d1L, d1I, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1I, order = 22) # 2.2e-16

grangertest(d1I, d1L, order = 10) # 1.329e-05

#4
ccf(d1L, d1T, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1T, order = 8) # < 2.2e-16

grangertest(d1T, d1L, order = 6) # 3.7e-05

#5
ccf(d1L, d1Mine, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1Mine, order = 14) # < 2.2e-16

grangertest(d1Mine, d1L, order = 10) # 5.191e-07

#6
ccf(d1L, d1U, lag.max = 27, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1U, order = 8) # < 2.2e-16

grangertest(d1U, d1L, order = 10) # 2.791e-07

#7
ccf(d1L, d1Cong, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1Cong, order = 8) # 2.2e-16 

grangertest(d1Cong, d1L, order = 10) # 0.0001237

#8
ccf(d1L, d110Y, lag.max = 14, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d110Y, order = 8) # 

grangertest(d110Y, d1L, order = 1) # 0.9748

#9
ccf(d1L, d1COil, lag.max = 18, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1COil, order = ) # - 

grangertest(d1COil, d1L, order = 15) # 5.674e-05

#10
ccf(d1L, d1S300, lag.max = 27, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1S300, order = 21) # 3.607e-11

grangertest(d1S300, d1L, order = 19) # 1.494e-05

#11
ccf(d1L, d1MSCI, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1MSCI, order = 8) # 2.2e-16

grangertest(d1MSCI, d1L, order = 10) # 5.191e-07 

#12
ccf(d1L, d1S100, lag.max = 26, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1S100, order = 8) # 2.2e-16

grangertest(d1S100, d1L, order = 9) # 5.507e-06 

#13
ccf(d1L, d1Comp, lag.max = 24, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1Comp, order = 8) # 2.2e-16

grangertest(d1Comp, d1L, order = 9) # 1.068e-05

#14
ccf(d1L, d1CNY, lag.max = 14, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1CNY, order = 3) # 0.04506

grangertest(d1Comp, d1L, order = 1) # 0.4494

#15
ccf(d1L, d1CPI, lag.max = 23, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1CPI, order = ) # -

grangertest(d1CPI, d1L, order = 9)# -

#16
ccf(d1L, d1Gold, lag.max = 14, type = c("correlation"),
    plot = TRUE)

grangertest(d1L, d1Gold, order = 12) # -

grangertest(d1Gold, d1L, order = 8) # -
...
grangertest(d1DJIA, d1L, order = 22) # -
...
grangertest(d1L, df_ex$ISF, order = 11) # -

grangertest(df_ex$ISF, d1L, order = 9) # -
...
df_ex <- df_ex %>% mutate (FC = 0.4863624*d110Y + 0.3578227*d1MSCI -0.7971289*d1Gold + 
                             0.06923116*d1Gold - 0.74601570*d1MSCI +0.66231988*d110Y)

exog <- data.frame(df_ex$ISF, df_ex$BCC, df_ex$FC)


Pacf(df_ex$ISF)
ur.df(df_ex$ISF, type="drift", lags = 1, 
      selectlags = "Fixed")

Pacf(df_ex$BCC)
ur.df(df_ex$BCC, type="drift", lags = 1, 
      selectlags = "Fixed")

Pacf(df_ex$FC)
ur.df(df_ex$FC, type="drift", lags = 1, 
      selectlags = "Fixed")
...
ARMAmodel <- Arima(data$LSha, c(0,1,1), include.constant =TRUE, method = c("CSS-ML"))  
...
summary(var2)
Hosking(var2, lags=1.124*var2$p) #0.8076918
LiMcLeod(var2, lags=1.124*var2$p) #0.948448


resvar2<-restrict(var2, method = c("ser"), thresh = 1)
summary(resvar2)
Hosking(resvar2, lags=1.35*var2$p) # 0.9324441
LiMcLeod(resvar2, lags=1.45*var2$p) # 0.9920119
...
var<-VAR(df1, p = 11, type = "const", exogen = exog)
...
ARMAmodelFC <- Arima(df_ex$FC, c(0,0,0), include.constant =TRUE, method = c("CSS-ML"))  
Box.test(residuals(ARMAmodelFC), lag = 7, type = c("Ljung-Box"), fitdf = 0) #p-value = 0.7527
...
forecastVARef <- predict(var, dumvar = exogf, n.ahead = 70, ci = 0.95)
plot.ts(forecastVARef$fcst$ydL)
write.csv(forecastVARef$fcst$ydL, "forecast_VARef_Sha.csv")
...
write.csv(forecast_trend$fcst$ydL, "forecast_VECM_trend_sha.csv")
...
write.csv(forecast_const$fcst$ydL, "forecast_VECM_const_sha.csv")
...
write.csv(dataER, "data_Shaanxi_forecast.csv")
...
MAPEef<-((abs((dataER$dL-dataER$VARef.fcst)/dataER$dL)))
